...
// InitCommand is used to initialize the shipwright application
type InitCommand struct {
	Force bool `short:"f" long:"force" description:"When this is set then the config file will be reinitialized"`
}

// Execute runs this command
func (i *InitCommand) Execute(config *shipwright.Config) {
	dpath := os.Getenv("HOME") + "/.shipwright"
	...
	if _, err := os.Stat(pth); os.IsNotExist(err) {
		...
		if strings.HasPrefix(strings.ToUpper(createYn), "N") {
			fmt.Print("Can't proceed without the configuration file ~/.shipwright/config.yml:")
			fmt.Println(`
---
common: &common
  caprica:
...
prod:
  ...
  `)
		...
		if _, err := os.Stat(os.Getenv("HOME") + "/.shipwright"); os.IsNotExist(err) {
